...
class ClassificationWorkflowTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.example = {"carat": 0.7, "cut": "Ideal", "color": "G", "depth": 61.5,
        ...
        for number in [1, 2]:
            ...
            save_best_model(model, "Tree", data["preprocessor"], number, "Verification model", {}, folder)
    ...
    def test_http_predictions_and_validation(self):
        ...
        request = Request(self.url + "/predict", data=json.dumps(payload).encode(),
    ...
    def test_exported_frames_match_model_inputs_and_labels(self):
        ...
        for number, data in self.prepared["experiments"].items():
            for split in ["train", "valid", "test"]:
                ...
                np.testing.assert_array_equal(frame[data["preprocessor"].get_feature_names_out()], data[f"X_{split}"])
                np.testing.assert_array_equal(frame["Clarity_Target"], self.prepared[f"y_{split}"])
                np.testing.assert_array_equal(frame["Source_Row"], self.prepared["diamonds"].index[self.prepared[f"{split}_indices"]])
