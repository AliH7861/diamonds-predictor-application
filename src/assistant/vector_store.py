...
class ChromaStores:
    ...
    def index_knowledge(self, knowledge_dir: str | Path) -> int:
        ...
        source = "".join(path.name + path.read_text(encoding="utf-8") for path in files)
        fingerprint = sha256(source.encode("utf-8")).hexdigest()[:16]
        chunks = [(path.name, chunk) for path in files for chunk in chunk_text(path.read_text(encoding="utf-8"))]
        ...
        if fingerprint not in stored_versions:
            ...
            self.knowledge.add(
                ...
                metadatas=[{"source": item[0], "version": fingerprint} for item in chunks],
                embeddings=self.embedder.embed([item[1] for item in chunks]),
            )
            return len(chunks)
        return 0
    ...
    def search_knowledge_details(self, queries: list[str], limit: int = 2) -> list[dict]:
        ...
        for query, vector in zip(queries, self.embedder.embed(queries)):
            ...
            for document, metadata, distance in zip(documents, metadatas, distances):
                ...
                if document not in found or candidate["similarity"] > found[document]["similarity"]:
                    found[document] = candidate
        return sorted(found.values(), key=lambda item: item["similarity"], reverse=True)
