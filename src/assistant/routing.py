...
def route_question(question: str, conversation_text: str = "") -> EvidenceRoute:
    ...
    if any(
        phrase in current for phrase in ("what would this cost", "predict price", "price estimate")
    ):
        return EvidenceRoute("price_prediction", use_knowledge=True, use_models=True)
    if any(
        phrase in current for phrase in ("predict clarity", "classify clarity", "clarity family")
    ):
        return EvidenceRoute("clarity_prediction", use_knowledge=True, use_models=True)
    if any(
        phrase in current for phrase in ("compare", "difference", "trade-off", "tradeoff", "why")
    ):
        return EvidenceRoute(
            "comparison",
            use_dataset=any(word in text for word in ("$", "budget", "carat")),
            use_knowledge=True,
            use_models="this diamond" in current,
        )
    buying_terms = (
        "i want",
        "find",
    ...
    continues_search = "saved search preferences:" in text and any(
        re.search(rf"\b{re.escape(term)}\b", current) for term in follow_up_terms
    )
    has_buying_term = any(
        (term == "$" and "$" in current)
    ...
    if has_buying_term or continues_search:
        needs_explanation = any(
            word in current for word in ("explain", "why", "trade-off", "tradeoff", "priorit")
        )
        return EvidenceRoute(
            "recommendation",
            use_dataset=True,
