...
from .feature_engineering import RAW_CLASSIFICATION_INPUTS, engineer_features
from .ordinal import predict_ordinal_ann, predict_ordinal_models
...
def save_best_model(model, model_type, preprocessor, experiment, model_name, validation_metrics, output_dir=DEFAULT_MODEL_DIR):
    ...
    model_file = "best_model.keras" if model_type in {"ANN", "OrdinalANN"} else "best_model.joblib"
    if model_type in {"ANN", "OrdinalANN"}:
    ...
    metadata = {
        ...
        "required_inputs": RAW_CLASSIFICATION_INPUTS,
        "representation": "Y23 physical-only; price excluded",
...
def load_best_model(model_dir=DEFAULT_MODEL_DIR):
    ...
    if metadata["model_type"] in {"ANN", "OrdinalANN"}:
...
def predict_diamonds(bundle, records):
    ...
    if model_type == "OrdinalANN":
        predictions, probabilities = predict_ordinal_ann(model, X)
        classes = np.arange(len(CLARITY_CLASSES))
    elif model_type == "OrdinalTree":
        predictions, probabilities = predict_ordinal_models(model, X)
        classes = np.arange(len(CLARITY_CLASSES))
    elif model_type == "ANN":
    ...
    if model_type not in {"OrdinalANN", "OrdinalTree"}:
